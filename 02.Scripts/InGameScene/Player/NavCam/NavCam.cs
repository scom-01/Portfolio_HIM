...
public class NavCam : MonoBehaviour
{
    ...
    void Rotate()
    {
        ...
        if (Input.GetKey(GlobalValue.Peeking_Left))    // KeyCode.Q))
        {
            Peeking_Left();
        }
        else if (Input.GetKey(GlobalValue.Peeking_Right))    // KeyCode.E))
        {
            Peeking_Right();
        }
        else if (!(Input.GetKey(GlobalValue.Peeking_Left) || Input.GetKey(GlobalValue.Peeking_Left)))
        {
            //부드럽게
            if (PeekingX != 0.0f)
            {
